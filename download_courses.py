...
# ! It should also be re-implemented with the higher level functions contained in the websis API
import mechanize
...
GenericParams = {"rsts": "dummy", "crn": "dummy",
                 ...
                 # "sel_ptrm": "%",
                 "sel_from_cred": "", "begin_ap": "x", "end_ap": "y", "path": "1", "SUB_BTN": "Course Search",
...
for option in tqdm.tqdm(soup.find("select").find_all('option')):
    ...
    for row in rows:
        ...
        ops = websis.get_options_for(
            sess, websis.CURRENT_TERM_ID, option["value"], current_course_info[0])
